...
class MySolver(nengo.solvers.Solver):
    ...
    # the old decoders
    def __init__(self, weights, my_weights):  # feed in old decoders
        super(MySolver, self).__init__(weights=weights)
...
def reset_gain_bias(P, model, sim, wm, wm_recurrent, wm_to_decision, drug):
    ...
    wm_recurrent.solver = MySolver(False, sim.model.params[wm_recurrent].weights)
    wm_to_decision.solver = MySolver(False, sim.model.params[wm_to_decision].weights)
